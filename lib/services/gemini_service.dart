...
class Geminservice {
  ...
  List<DishModel> parseCustomJsonString(String jsonString) {
    try {
      ...
      throw Exception("mauvais format");
    }
  }

  Future<List<DishModel>> GetMEnuByBudget(
    String budget,
    int nbPersonne,
    String prefferences,
  ) async {
    ...
    try {
      ...
      NB: . la somme des prix unitaire des ingrédients * la quantite doit être inférieure a  $budget Fcfa.
          . les propositions que tu fera prendrons en compte le fait que c'est pour $nbPersonne personnes,
          . et de prefference priviligies les plats $prefferences
      ...
      return parseCustomJsonString(response.output as String);
    } catch (error) {
  ...
  void getItemsWithOrder(List<dynamic> items, void Function(String) callback) {
    gemini
        .prompt(
          parts: [
            Part.text('''
