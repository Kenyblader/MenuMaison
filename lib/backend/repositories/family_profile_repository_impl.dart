...
class FamilyProfileRepositoryImpl implements FamilyProfileRepository {
  static final FamilyProfileRepositoryImpl _instance = FamilyProfileRepositoryImpl._internal();
  ...
  Future<bool> isProfileConfigured() async {
    ...
    final List<Map<String, dynamic>> profiles = await db.query(_tableFamilyProfiles);
  ...
  Future<void> saveProfile(FamilyProfileModel profile) async {
    final db = await DatabaseHelper().database;
    await db.insert(_tableFamilyProfiles, FamilyProfileEntity(
  ...
  Future<FamilyProfileModel?> getProfile() async {
    ...
    if (profiles.isNotEmpty) {
      return FamilyProfileModel.fromEntity(FamilyProfileEntity.fromMap(profiles.first));
    }
    return null;
  }
}
